...
const KEYWORD_PATTERNS: Array<{
  ...
  { re: /\b(escut|ouvir\s+atent|ouv\w*\s+(?:os|o)\s|notar|perceb|atentar|observar\s+(?:a\s+|o\s+)?(?:sala|ambiente|arredor|sombras))/i,
  ...
  { re: /\b(escal|saltar|salto|balan[çc]|trep|nada|nadar|arrast|empurrar\s+(?:a|o)\s+pedra|levantar\s+(?:a\s+|o\s+)?(?:port|gigante|gradem))/i,
